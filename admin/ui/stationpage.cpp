    ...
    m_client->sendRequest(QStringLiteral("station_list"), QJsonObject(),
                          [this](int code, const QString &, const QJsonObject &data) {
                              ...
                              for (int row = 0; row < stations.size(); ++row) {
                                  ...
                                  m_table->setItem(row, 5, new QTableWidgetItem(QString::number(s[QStringLiteral("pileTotal")].toInt())));
                                  m_table->setItem(row, 6, new QTableWidgetItem(QStringLiteral("%1%").arg(s[QStringLiteral("onlineRate")].toDouble() * 100, 0, 'f', 0)));
                              }
                          });
}
...
void StationPage::onShowPiles()
{
    ...
    m_client->sendRequest(QStringLiteral("pile_list"), QJsonObject{{QStringLiteral("stationId"), stationId}},
                          [this](int code, const QString &msg, const QJsonObject &data) {
                              ...
                              QDialog dialog(this);
                              dialog.setWindowTitle(QStringLiteral("站内电桩"));
...
void StationPage::onAddStation()
{
    ...
    if (nameEdit->text().trimmed().isEmpty() || lngEdit->text().isEmpty() || latEdit->text().isEmpty()) {
        QMessageBox::warning(this, QStringLiteral("新增站点"), QStringLiteral("站名、经度、纬度不能为空"));
        return;
    }
                          ...
                          [this](int code, const QString &msg, const QJsonObject &data) {
                              if (code == 0) {
                                  QMessageBox::information(this, QStringLiteral("新增站点"),
                                                           QStringLiteral("创建成功，站点ID：%1").arg(data[QStringLiteral("stationId")].toInt()));
                                  refresh();
                              } else {
