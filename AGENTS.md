...
仓库处于初始化阶段：`docs/` 含通信协议、项目说明书、需求矩阵（已含评审反馈新增 NO.28~32）与视觉规范；`admin/` 管理端五个功能页已实现并通过构建/协议/界面验证（见 `feat/admin` 分支）。按 NO.28~30，站点修改/删除（逻辑删除）/分页搜索为下一阶段任务，需先完成协议补充（NO.32，与宋昊润确认）再动 UI。创建新模块目录时遵循上表结构，不要另起顶层目录。
