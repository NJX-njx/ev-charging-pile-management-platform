...
`docs/` 含通信协议、项目说明书、需求矩阵（已含评审反馈新增 NO.28~32）与视觉规范。`server/`（多线程 Socket 服务端 + HTTP 只读接口）、`client/`（Qt 用户端：登录/找站/充电/我的）、`admin/`（Qt 管理端五个功能页，已通过构建/协议/界面验证）三个模块均已完成第一阶段开发并合入 `main`。按 NO.28~30，站点修改/删除（逻辑删除）/分页搜索为下一阶段任务，需先完成协议补充（NO.32）再动 UI。创建新模块目录时遵循上表结构，不要另起顶层目录。
