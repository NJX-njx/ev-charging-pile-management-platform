...
`docs/` 含通信协议、项目说明书、需求矩阵（共 51 项，按模块归组：NO.1~3 项目基础、4~18 充电用户端、19~41 运营管理端、42~48 服务端、49~50 Web 数据展示、51 项目收尾；9 月 5 日修订：补全电桩/用户增删改、管理端订单查询、管理员与用户密码等条目，用户登录改为「手机号+密码」或「手机号+模拟验证码」，矩阵 NO.4/46 对应，需先改 protocol.md 再动代码）与视觉规范。`server/`（多线程 Socket 服务端 + HTTP 只读接口）、`client/`（Qt 用户端：登录/找站/充电/我的）、`admin/`（Qt 管理端五个功能页，已通过构建/协议/界面验证）三个模块均已完成第一阶段开发并合入 `main`。按需求矩阵，站点修改/删除/分页搜索（NO.32~34）为下一阶段任务，需先完成服务端支持（NO.47）再动 UI。创建新模块目录时遵循上表结构，不要另起顶层目录。
