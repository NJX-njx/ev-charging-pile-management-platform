...
仓库处于初始化阶段：`docs/` 含通信协议、项目说明书与需求矩阵；管理端骨架（登录窗口、主界面框架、Socket 通信封装）正在 `feat/admin` 分支开发，尚未合入；`client/` Qt 用户端（登录/找站/充电/我的）已在 `feat/client` 分支实现。创建新模块目录时遵循上表结构，不要另起顶层目录。
