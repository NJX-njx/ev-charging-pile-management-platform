...
`docs/` 含通信协议、项目说明书、需求矩阵（共 51 项，按模块归组：NO.1~3 项目基础、4~18 充电用户端、19~41 运营管理端、42~48 服务端、49~50 Web 数据展示、51 项目收尾；9 月 5 日修订：补全电桩/用户增删改、管理端订单查询、管理员与用户密码等条目，用户登录改为「手机号+密码」或「手机号+模拟验证码」）与视觉规范。`server/`（多线程 Socket 服务端 + HTTP 只读接口）、`client/`（Qt 用户端：登录/找站/充电/我的）、`admin/`（Qt 管理端六个功能页）三个模块均已完成第一阶段开发并合入 `main`，且已按**协议 v2**（密码/验证码双认证、电桩/站点/用户增改删、站点分页搜索、管理端订单查询与详情、逻辑删除、错误码 1005）完成三端适配并联调通过（服务端 687 项断言冒烟）。剩余：Web 大屏前端页面（NO.49/50）与项目收尾（NO.51）。创建新模块目录时遵循上表结构，不要另起顶层目录。
